...
2. **`docker compose`** (`docker-compose.yml`) — same image plus the
   ...
   writable rootfs.
3. **`podman compose -f docker/compose.yaml`** — the hardened path. Rootless
...
read-only root filesystem — `docker/compose.yaml` is the supported starting
...
podman compose -f docker/compose.yaml logs -f deeptutor
```

Verify rootless is active (`podman info | grep -i rootless` should
report `true`).

What `docker/compose.yaml` does, and why:
...
`docker/compose.yaml` is a starting point, not the only shape. The same
...
- **`system.json` → `backend_port` / `frontend_port`**. The container
  ...
  `HOST_PORT_*` env var that `docker/compose.yaml` reads) to match.
...
The `pocketbase` service in `docker/compose.yaml` (and the corrected mount in
...
- The sandbox-runner sidecar (in `docker-compose.yml`, **not** in
  `docker/compose.yaml`) is the strongest posture for untrusted model-generated
